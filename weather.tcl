...
# // Checks if there are multiple choices for the city eg. "detroit" which has 6 entries     
  ...
  if {[info exists loc]==0} { 
	putquick "PRIVMSG $chan :Condiçõees não disponíveis para $input. Tente refinar a busca adicionando o estado ou país."
    return 0
  }
...
# // Wind Direction, Wind Speed. Reverse ${windm}/${windk} and ${windgm}/${windgk} if you want metric as priority
  ...
  if {$windm==0} { 
    ...
    set complement ";"
    if {$windk < $windgk} {
		set complement " com rajadas de até ${windgk}km/h;"
	} 
...
# // Reverse ${tempf}/${tempc} and ${windcf}/${windcc} if you want metric as priority
  ...
  if {(([info exists windcf]==1)&&(abs($tempf - $windcf) > 1.0))&&($tempf > $windcf)} {
    ...
    putquick "PRIVMSG $chan :\00313$loc:\003 \002Temperatura:\002$colorf ${tempc}C\003; \002Sensação térmica:\002$colorw ${windcc}C\00300; \002Umidade:\002 $hum; \002Vento:\002 $windout\ \002Condições:\002 $brCondition; \002Atualizado:\002 $updated"
  }  else {
    putquick "PRIVMSG $chan :\00313$loc:\003 \002Temperatura:\002$colorf ${tempc}C\003; \002Umidade:\002 $hum; \002Vento:\002 $windout \002Condições:\002 $brCondition; \002Atualizado:\002 $updated"
  }
}
